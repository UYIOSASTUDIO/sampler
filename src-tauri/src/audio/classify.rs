pub fn classify_by_filename(filename: &str) -> Option<String> {
    ...
    if name.contains("perc") || name.contains("tom") || name.contains("bongo") || name.contains("shaker") {
        return Some("Percussion".to_string());
    }
    if name.contains("bass") || name.contains("808") || name.contains("sub") || name.contains("reese") {
        return Some("Bass".to_string());
    }
    if name.contains("vocal") || name.contains("vox") || name.contains("acapella") || name.contains("chant") {
        return Some("Vocal".to_string());
    }
    if name.contains("fx") || name.contains("riser") || name.contains("impact") || name.contains("sweep") || name.contains("noise") || name.contains("sfx") {
        return Some("FX".to_string());
    }
    if name.contains("synth") || name.contains("pad") || name.contains("lead") || name.contains("chord") || name.contains("arp") || name.contains("pluck") {

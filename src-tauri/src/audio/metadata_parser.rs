...
pub fn parse_filename(filename: &str, is_loop: bool) -> ParsedMetadata {
    ...
    let bpm_explicit = BPM_EXPLICIT.get_or_init(|| {
        Regex::new(r"(?i)\b(\d{2,3}(?:\.\d+)?)\s*[-_]?\s*bpm\b").unwrap()
    });
    ...
    if bpm.is_none() && is_loop {
        ...
        let bpm_delimited = BPM_IMPLICIT_DELIMITED.get_or_init(|| {
            Regex::new(r"[-_]\s*(\d{2,3})\s*(?:[-_\.]|$)").unwrap()
        });
        ...
        let bpm_3digit = BPM_IMPLICIT_3DIGIT.get_or_init(|| {
            Regex::new(r"(?:^|\s)(\d{3})(?:\s|\.|$)").unwrap()
        });
    ...
    let key_short = KEY_SHORT.get_or_init(|| {
        Regex::new(r"\b([A-G][#b]?)(m|M)\b").unwrap()
    });

    static CAMELOT: OnceLock<Regex> = OnceLock::new();
    let camelot = CAMELOT.get_or_init(|| {
        Regex::new(r"(?i)\b(1[0-2]|[1-9])[AB]\b").unwrap()
    });
    ...
    let key_dashed = KEY_DASHED.get_or_init(|| {
        Regex::new(r"(?i)\s-\s([A-G][#b]?)\s-\s").unwrap()
    });

    if let Some(caps) = key_explicit.captures(filename) {
        let note = caps[1].to_uppercase();
        let mode = caps[2].to_lowercase();
        let mode_formatted = if mode.starts_with("min") { "min" } else { "maj" };
        ...
        if let Some(mode_match) = caps.get(2) {
            ...
            let mode_formatted = if mode.starts_with("m") && mode != "maj" && mode != "major" { "min" } else { "maj" };
            key = Some(format!("{} {}", note, mode_formatted));
        } else {
            key = Some(note);
        }
